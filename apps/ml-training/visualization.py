...
def plot_predictions_vs_actual(sample_df, output_file="predictions_vs_actual.png"):
    ...
    ax = axes[0]
    ax.scatter(sample_df["actual_gpu_energy_j"], 
    ...
    ax.plot([min_val, max_val], [min_val, max_val], 'k--', lw=2, alpha=0.7, label='Perfect prediction')
    ...
    ax = axes[1]
    ax.scatter(sample_df["actual_output_tokens"], 
               sample_df["pred_output_tokens"], 
               alpha=0.6, s=50, color='#4ECDC4', edgecolors='black', linewidth=0.5)
...
def plot_residuals(sample_df, output_file="residuals.png"):
    ...
    ax.scatter(sample_df["pred_gpu_energy_j"], residuals_energy, 
               alpha=0.6, s=50, color='#FF6B6B', edgecolors='black', linewidth=0.5)
    ...
    ax = axes[1]
    residuals_tokens = sample_df["actual_output_tokens"] - sample_df["pred_output_tokens"]
    ax.scatter(sample_df["pred_output_tokens"], residuals_tokens,
               alpha=0.6, s=50, color='#4ECDC4', edgecolors='black', linewidth=0.5)
    ax.axhline(y=0, color='k', linestyle='--', lw=2)
...
def plot_energy_vs_tokens(sample_df, output_file="energy_vs_tokens.png"):
    ...
    # Create scatter plot
    scatter = ax.scatter(sample_df["input_tokens"], 
    ...
    ax.set_title('Inference Cost: Input Tokens vs Energy', fontweight='bold', fontsize=14)
    ax.grid(alpha=0.3)
...
def plot_error_distribution(sample_df, output_file="error_distribution.png"):
    ...
    # GPU Energy errors
    ax = axes[0]
    errors_energy = np.abs(sample_df["actual_gpu_energy_j"] - sample_df["pred_gpu_energy_j"])
    ...
    ax.axvline(errors_energy.mean(), color='red', linestyle='--', lw=2, label=f'Mean: {errors_energy.mean():.2f}')
    ax.legend()
    ax.grid(alpha=0.3, axis='y')
    
    # Output tokens errors
    ax = axes[1]
    errors_tokens = np.abs(sample_df["actual_output_tokens"] - sample_df["pred_output_tokens"])
    ...
    ax.axvline(errors_tokens.mean(), color='teal', linestyle='--', lw=2, label=f'Mean: {errors_tokens.mean():.2f}')
    ax.legend()
    ax.grid(alpha=0.3, axis='y')
...
def plot_model_architecture(output_file="model_architecture.png"):
    ...
    draw_box(ax, 0.8, 5.3, 2, 0.8, 'TF-IDF Vectorizer\n(2000 features)', process_color, fontsize=10)
    ...
    draw_box(ax, 2, 2.1, 3, 0.8, 'MultiOutputRegressor (Ridge)', process_color, fontsize=11)
    ...
    # Legend
    ax.text(6.5, 7.5, '📊 Multi-output Regression', fontsize=10, 
