...
def create_comprehensive_dashboard(sample_df, output_file="dashboard.png"):
    ...
    ax_title.axis('off')
    ax_title.text(0.5, 0.8, 'GPU Energy & Token Prediction Model', 
                 fontsize=20, fontweight='bold', ha='center', transform=ax_title.transAxes)
    ax_title.text(0.5, 0.3, 'Multi-output regression predicting GPU energy consumption and output tokens from input text and token count',
                 fontsize=11, ha='center', transform=ax_title.transAxes, style='italic', color='gray')
    
    # ===== ROW 2: PREDICTIONS & ERRORS =====
    # Predictions plot - GPU Energy
    ax1 = fig.add_subplot(gs[1, 0])
    ax1.scatter(sample_df["actual_gpu_energy_j"], 
               sample_df["pred_gpu_energy_j"], 
               alpha=0.5, s=30, color='#FF6B6B', edgecolors='darkred', linewidth=0.3)
    ...
    ax2.scatter(sample_df["actual_output_tokens"], 
               sample_df["pred_output_tokens"],
               alpha=0.5, s=30, color='#4ECDC4', edgecolors='darkgreen', linewidth=0.3)
    ...
    rmse_energy = np.sqrt(mean_squared_error(sample_df["actual_gpu_energy_j"], sample_df["pred_gpu_energy_j"]))
    rmse_tokens = np.sqrt(mean_squared_error(sample_df["actual_output_tokens"], sample_df["pred_output_tokens"]))
    ...
    ax4.hist(errors_e, bins=25, color='#FF6B6B', alpha=0.7, edgecolor='darkred', linewidth=0.5)
    ax4.axvline(errors_e.mean(), color='red', linestyle='--', lw=2, label=f'μ={errors_e.mean():.2f}')
    ...
    errors_t = np.abs(sample_df["actual_output_tokens"] - sample_df["pred_output_tokens"])
    ax5.hist(errors_t, bins=25, color='#4ECDC4', alpha=0.7, edgecolor='darkgreen', linewidth=0.5)
    ax5.axvline(errors_t.mean(), color='teal', linestyle='--', lw=2, label=f'μ={errors_t.mean():.2f}')
    ...
    scatter = ax6.scatter(sample_df["input_tokens"],
                         sample_df["actual_gpu_energy_j"],
                         c=sample_df["actual_output_tokens"],
                         s=50, cmap='plasma', alpha=0.6, edgecolors='black', linewidth=0.3)
...
def create_summary_infographic(sample_df, output_file="summary_infographic.png"):
    ...
    # Title
    ax.text(5, 9.3, 'Sustainability ML Model', fontsize=24, fontweight='bold', ha='center')
    ax.text(5, 8.9, 'Predicting GPU Energy Consumption & LLM Output', fontsize=12, ha='center', style='italic', color='gray')
    ...
    # Box 1: Dataset
    rect = FancyBboxPatch((0.5, box_y - box_height), box_width, box_height,
                          boxstyle="round,pad=0.1", facecolor='#E3F2FD', 
                          edgecolor='#1976D2', linewidth=2.5)
    ax.add_patch(rect)
    ax.text(0.5 + box_width/2, box_y - 0.3, '📊 Dataset', fontsize=12, fontweight='bold', ha='center')
    ax.text(0.5 + box_width/2, box_y - 0.65, f'{len(sample_df)} samples', fontsize=11, ha='center', fontweight='bold')
    ax.text(0.5 + box_width/2, box_y - 0.95, '80/20 split', fontsize=10, ha='center', style='italic')
    
    # Box 2: Model Accuracy
    rect = FancyBboxPatch((3.6, box_y - box_height), box_width, box_height,
                          boxstyle="round,pad=0.1", facecolor='#F3E5F5',
                          edgecolor='#7B1FA2', linewidth=2.5)
    ax.add_patch(rect)
    ax.text(3.6 + box_width/2, box_y - 0.3, '🎯 Accuracy', fontsize=12, fontweight='bold', ha='center')
    ax.text(3.6 + box_width/2, box_y - 0.65, f'R² = {r2_energy:.3f}', fontsize=11, ha='center', fontweight='bold')
    ax.text(3.6 + box_width/2, box_y - 0.95, f'MAE = {mae_energy:.2f}J', fontsize=10, ha='center', style='italic')
    
    # Box 3: Features
    rect = FancyBboxPatch((6.7, box_y - box_height), box_width, box_height,
                          boxstyle="round,pad=0.1", facecolor='#E8F5E9',
                          edgecolor='#388E3C', linewidth=2.5)
    ax.add_patch(rect)
    ax.text(6.7 + box_width/2, box_y - 0.3, '⚙️ Features', fontsize=12, fontweight='bold', ha='center')
    ax.text(6.7 + box_width/2, box_y - 0.65, '2000 TF-IDF', fontsize=10, ha='center', fontweight='bold')
    ax.text(6.7 + box_width/2, box_y - 0.95, '+ token count', fontsize=10, ha='center', style='italic')
    ...
    for text, x, color in steps:
        circle = Circle((x, y_line - 1.2), 0.35, color=color, ec='black', linewidth=2, zorder=2)
        ax.add_patch(circle)
        ax.text(x, y_line - 1.2, text, fontsize=8, ha='center', va='center', 
               fontweight='bold', zorder=3)
        
        # Draw arrow between circles
        if x < 7.2:
            ax.annotate('', xy=(x + 0.6, y_line - 1.2), xytext=(x + 0.35, y_line - 1.2),
                       arrowprops=dict(arrowstyle='->', lw=2, color='black'))
    
    # Output targets
    y_output = 3.5
    ax.text(5, y_output, 'Prediction Targets', fontsize=14, fontweight='bold', ha='center')
    ...
    for text, x, color in output_items:
        rect = FancyBboxPatch((x - 0.9, y_output - 1.3), 1.8, 0.7,
    ...
    # Footer
    ax.text(5, 0.5, '🌱 Reducing AI Carbon Footprint Through Predictive Energy Modeling 🌱',
